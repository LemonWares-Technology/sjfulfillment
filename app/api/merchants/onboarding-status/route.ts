...
// Allowed onboarding statuses as string union
const allowedStatuses = ["PENDING", "APPROVED", "REJECTED", "SUSPENDED"] as const;
type OnboardingStatus = typeof allowedStatuses[number];
function toOnboardingStatus(val: string): OnboardingStatus {
  return allowedStatuses.includes(val as OnboardingStatus) ? (val as OnboardingStatus) : "PENDING";
}
  ...
  async (
    ...
    try {
      ...
      return createResponse(
        updatedMerchant,
        200,
        `Merchant onboarding status updated to ${updateData.status}`
      );
    } catch (error) {
      ...
      return createErrorResponse(
        "Failed to update merchant onboarding status",
        500
      );
    }
  }
...
export const GET = withRole(
  ...
  async (request: NextRequest, user: JWTPayload) => {
    try {
      ...
      const statusEnum = toOnboardingStatus(statusRaw);
      const skip = (page - 1) * limit;

      const [merchants, total] = await Promise.all([
        prisma.merchant.findMany({
          where: { onboardingStatus: statusEnum },
          skip,
          take: limit,
          include: {
            users: {
        ...
        prisma.merchant.count({ where: { onboardingStatus: statusEnum } }),
      ]);
